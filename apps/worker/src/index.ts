...
async function reconcileStuckJobs(): Promise<void> {
  ...
  const cutoff = Date.now() - runtimeConfig.recoveryStaleMs;
  ...
  const staleExports = await db
    ...
    .where(
      and(
        ...
        isNotNull(exportsTable.processingStartedAt),
      ),
    );
  ...
  const staleSources = await db
    ...
    .where(
      and(
        ...
        isNotNull(sourcesTable.processingStartedAt),
      ),
    );
...
void startWorkers().catch((error) => {
  logger.error("Worker startup validation failed", error);
  process.exit(1);
});
