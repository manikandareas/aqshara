      ...
      <aside className="w-[260px] shrink-0 bg-sidebar border-r border-sidebar-border text-sidebar-foreground flex flex-col justify-between h-full">
        ...
        <div className="flex flex-col gap-1 p-3">
          ...
          <div className="space-y-[2px]">
            <NavItem icon={<PlusIcon filled />} label="New" active />
            <NavItem icon={<DocumentIcon />} label="Documents" />
            <NavItem icon={<LibraryIcon />} label="Library" />
            <NavItem icon={<SparklesIcon />} label="AI Chat" />
          </div>
        </div>
...
function NavItem({ icon, label, active = false }: { icon: React.ReactNode, label: string, active?: boolean }) {
  return (
    <button className={`w-full flex items-center gap-3 px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${
      active 
